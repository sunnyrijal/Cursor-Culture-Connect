...
export const store = {
  ...
  getConnections: (userId: string): User[] => {
    ...
    return state.users.filter((user) => connectionIds.includes(user.id.toString()));
  },
  ...
  createGroup: (groupData: Omit<Group, 'id' | 'memberCount' | 'isJoined' | 'recentActivity' | 'upcomingEvents'>) => {
